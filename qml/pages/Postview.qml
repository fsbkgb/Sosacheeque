    ...
    SilicaListView {
        ...
        Component {
            ...
            ContextMenu {
                ...
                MenuItem {
                    text: qsTr("Reply")
                    onClicked: pageStack.push(Qt.resolvedUrl("Newpost.qml"), {domain: domain, board: board, thread: trd, comment: comm } )
                }
                MenuItem {
                    text: qsTr("Reply with quote")
                    onClicked: {
                        var q = comm + "\r\n>" + quote.replace(/<br>/g, "\r\n>").replace(/<\/?[^>]+>/g, "")
                        pageStack.push(Qt.resolvedUrl("Newpost.qml"), {domain: domain, board: board, thread: trd, comment: q } )
                    }
                }
            }
        }
